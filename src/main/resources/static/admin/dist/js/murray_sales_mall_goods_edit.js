    ...
    new AjaxUpload('#uploadGoodsCoverImg', {
        ...
        onSubmit: function (file, extension) {
            if (!(extension && /^(jpg|jpeg|png|gif)$/.test(extension.toLowerCase()))) {
                Swal.fire({
                    text: "Only Support jpg、png、gif file format！",
                    icon: "error", iconColor: "#f05b72",
                });
                return false;
            }
...
$('#saveButton').click(function () {
    ...
    if (!validLength(goodsName, 100)) {
        Swal.fire({
            text: "Goods name too long, please enter less than 100 characters",
            icon: "error", iconColor: "#f05b72",
        });
        return;
    }
    ...
    if (!validLength(tag, 100)) {
        Swal.fire({
            text: "Tag too long, please enter less than 100 characters",
            icon: "error", iconColor: "#f05b72",
        });
        return;
    }
    if (isNull(goodsIntro)) {
        Swal.fire({
            text: "Enter the product introduction",
            icon: "error", iconColor: "#f05b72",
        });
        return;
    }
    if (!validLength(goodsIntro, 100)) {
        Swal.fire({
            text: "Introduction too long, please enter less than 100 characters",
            icon: "error", iconColor: "#f05b72",
        });
        return;
    }
    if (isNull(originalPrice) || originalPrice < 1) {
        Swal.fire({
            text: "Enter the original price of the product",
            icon: "error", iconColor: "#f05b72",
        });
        return;
    }
    if (isNull(sellingPrice) || sellingPrice < 1) {
        Swal.fire({
            text: "Enter the selling price of the product",
            icon: "error", iconColor: "#f05b72",
        });
        return;
    }
    ...
    if (isNull(goodsSellStatus)) {
        Swal.fire({
            text: "Select the product selling status",
            icon: "error", iconColor: "#f05b72",
        });
        return;
    }
    ...
    if (!validLength(goodsDetailContent, 50000)) {
        Swal.fire({
            text: "Products description too long",
            icon: "error", iconColor: "#f05b72",
        });
        return;
    }
    if (isNull(goodsCoverImg) || goodsCoverImg.indexOf('img-upload') != -1) {
        Swal.fire({
            text: "Cover image cannot be empty and must be uploaded",
            icon: "error", iconColor: "#f05b72",
        });
        return;
    }
    ...
    if (goodsId > 0) {
        ...
        swlMessage = 'Modify successfully';
        data = {
            "goodsId": goodsId,
            "goodsName": goodsName,
            "goodsIntro": goodsIntro,
        ...
        success: function (result) {
            if (result.resultCode == 200) {
                Swal.fire({
                    ...
                    confirmButtonText: 'Return to the product list',
